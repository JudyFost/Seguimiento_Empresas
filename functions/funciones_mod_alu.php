...
//Uso de la condicional if para realizar la búsqueda de un usuario por idusuario, nombre, ap_paterno, apellido materno y puesto
    ...
    if($tipoopcion=="id_usuario"){
	  $tipo_usuario_idtipo=$_POST['$tipo_usuario_idtipo'];
	   $sql="SELECT * FROM usuario where tipo_usuario_idtipo LIKE '$txtabuscar%' AND idusuario='$idusuario_user' order by idusuario;";
...
$consulta=mysql_query($sql);		
        //Inicio tabla responsiva para la consulta de los usuarios
          echo '<table border="0" class="table table-responsive table-hover">';
		    echo '<th>No. Control</th><th>Nombre(s)</th><th>Apellido Paterno</th><th>Apellido Materno</th><th>Status</th><th>Ver</th><th>Editar</th>';
...
$consulta=mysql_query($sql);                 
                ...
                $idusuario=$row['idusuario'];	
                                      ...
                                      echo '        </div>';
                                      echo '        <div class="modal-body">';
                                      include ('../forms/form_actualizar_alu.php');  ///incluyo el formulario
                                      echo '        </div>';
                                      echo '      </div>';
                                      echo '    </div>';
                                      echo '  </div>';
