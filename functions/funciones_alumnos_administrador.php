<!--Método para invocar la sesion de un alumno antes de manipularlo y si ya esta el empresa creada entonces manipular la información-->
<?php
session_start();
...
if($_POST['txtabuscar']!=NULL){
	...
	if($tipoopcion=="puesto"){
		...
		while ($rowcon=mysql_fetch_array($consultacon)){
			...
			echo "<center>";
				echo '<div class="alert alert-danger">';
				echo '<strong>¡Error!</strong> Esté status no existe, Verifique sus datos';
				echo '</div>';
				exit();
			}
		}
...
$consulta=mysql_query($sql);		
        //Inicio tabla responsiva para la consulta de los usuarios
          echo '<table border="0" class="table table-responsive table-hover">';
		    ...
		    echo '<th>No. Control</th><th>Nombre(s)</th><th>Apellido Paterno</th><th>Apellido Materno</th><th>Status</th><th>Ver</th><th>Editar</th><th>Eliminar</th>';
                  ...
                  echo '<tr>';
					...
					echo '<br>';
					    include("../include/conexion.php");
						   ...
						   if($row['tipo_usuario_idtipo']=="2"){
								$resultado = mysql_query("SELECT * FROM vista_alumno WHERE idusuario='$idusuario' order by idusuario;");
								if (!$resultado) {
                    ...
                    if(isset($_SESSION['logueo'])){
                          if($_SESSION['logueo']=='t'){
								...
								echo '<td>';
								    ...
								    echo '</button>';    
								echo '</td>';
                                //mas informacion del usuario alumno que solo el usuario registrado podra ver.
                                //    <!-- Modal -->
                                ...
                                echo '        <h4 class="modal-title">'.'<font color="gray"><b><center>'."No. CONTROL:"." ".$row['idusuario'].'</b></font>'.'</center>'.'</h4>';
                                echo '      </div>';
                                echo '      <div class="modal-body">';
                                echo '      <p><b>'."No. CONTROL:".'</b>'." ".utf8_encode($row['idusuario']).'<br>'."<b>ALUMNO (A): </b>"." ".utf8_encode($row['nombre'])." ".utf8_encode($row['ap_paterno'])." ".utf8_encode($row['ap_materno']);
                                echo '        <b>'."<br>STATUS: </b>"." ".utf8_encode($row['puesto']);
								echo '        <b>'."<br>SEXO: ".'</b>';
												  $sexo=$row['sexo'];
												  if($sexo=='H'){
												  echo 'HOMBRE';
...
$consulta=mysql_query($sql);                 
                ...
                $idusuario=$row['idusuario'];	
                                      ...
                                      echo '        <div class="modal-header">';
                                      echo '          <button type="button" class="close" data-dismiss="modal">&times;</button>';
                                      echo '          <h5 class="modal-title" style="color:black;"><b><center>ACTUALIZAR ALUMNO</center></b></h5>';
                                      echo '        </div>';
                                      echo '        <div class="modal-body">';
                                      include ('../forms/form_actualizar_alumno.php');  ///incluyo el formulario
                                      echo '        </div>';
                                      echo '      </div>';
                                      echo '    </div>';
                                      echo '  </div>';
...
$consulta=mysql_query($sql);                 
                ...
                $idusuario=$row['idusuario'];
                                      ...
                                      echo ' <!--Ventana para captura de eliminar alumno-->';
                                      ...
                                      echo '        <div class="modal-header">';
                                      echo '          <button type="button" class="close" data-dismiss="modal">&times;</button>';
                                      echo '          <h5 class="modal-title" style="color:black;"><b><center>ELIMINAR ALUMNO</center></b></h5>';
                                      echo '        </div>';
                                      echo '        <div class="modal-body">';
                                      include ('../forms/form_eliminar_alumno.php');  ///incluyo el formulario
                                      echo '        </div>';
                                      echo '      </div>';
                                      echo '    </div>';
                                      echo '  </div>';
